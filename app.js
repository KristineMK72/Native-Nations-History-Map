...
import { PLACES } from "./data/places.js";// /app.js
import { CHAPTERS } from "./data/chapters.js";
import { TRAILS } from "./data/trails.js";
...
function renderSources(sources = []) {
  ...
  const items = sources
    ...
    .map(
      (s) =>
        `<a href="${escapeHtml(s.href)}" target="_blank" rel="noopener noreferrer">${escapeHtml(
          s.label
        )}</a>`
    );
...
function renderPopupImage(image) {
  if (!image?.src) return "";
      ...
      ${
        creditLine
          ? `<div class="popup-credit">
              ${
                href
                  ? `<a href="${href}" target="_blank" rel="noopener noreferrer">${creditLine}</a>`
                  : creditLine
              }
            </div>`
          : ""
...
========================================================= */
async function fetchArcGisGeoJsonAll(
  urlBase,
...
function setupLayers() {
  ...
  // Places (with image + sources support)
  ...
  for (const p of PLACES) {
    ...
    L.circleMarker([p.lat, p.lng], {
      ...
      .bindPopup(
        `
        <div class="popup" style="min-width:240px; max-width:320px;">
          <strong>${name}</strong>
          ${type ? `<div style="opacity:.75; font-size:12px; margin-top:4px;">Type: ${type}</div>` : ""}
          ${renderPopupImage(p.image)}
...
/* =========================================================
   Story (sidebar) render (with chapter images)
========================================================= */
      ...
      ? `
        <figure class="chapter-figure">
          ...
          <figcaption class="chapter-credit">
            ${
              img.href
                ? `<a href="${escapeHtml(img.href)}" target="_blank" rel="noreferrer noopener">
